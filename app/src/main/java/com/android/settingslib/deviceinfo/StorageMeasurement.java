...
public class StorageMeasurement {
        ...
        if (this.mSharedVolume != null && this.mSharedVolume.isMountedReadable()) {
            ...
            if (this.mSharedVolume.getType() == 2) {
                for (UserInfo user : users) {
                    addValue(details.usersSize, user.id, getDirectorySize(imcs, this.mSharedVolume.getPathForUser(user.id)));
                }
            }
        }
    ...
    private static long getDirectorySize(IMediaContainerService imcs, File path) {
        try {
            ...
            if (LOGV) {
                Log.v(TAG, "getDirectorySize(" + path + ") returned " + size);
            }
            return size;
        } catch (Exception e) {
